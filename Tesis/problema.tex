...
\WHILE {Respuesta == Some}
\STATE Acciones habilitadas = \{a$_{1}$, ..., a$_{k}$\} tal que \{a$_{1}$, ..., a$_{k}$\} $\subseteq$ acciones(View[0]) $\cup$ acciones individuales $\cup$ acciones compartidas disponibles
...
\ENSURE La respuesta decide la realizabilidad de Goal sobre las implementaciónes de Knowledge, tanto si el estado inicial de Knowledge es el estado actual o si es el estado en donde comienza la exploración
\STATE Knowledge en posición actual = cambiar estado inicial a actual(Knowledge)
\STATE Respuesta = Sintetizar sin tomar en cuenta el estado inicial(Knowledge en posición actual, Goal)
\IF{Respuesta == None}
\STATE Respuesta = Sintetizar sin tomar en cuenta el estado inicial(Knowledge, Goal)
\IF{Respuesta == None}
\RETURN None
\ELSE
\RETURN Reset
